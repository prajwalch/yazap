...
pub fn init(allocator: Allocator, cmd_name: []const u8) Yazap {
    return Yazap{
        .allocator = allocator,
        .command = Command.new(allocator, cmd_name),
    };
}
...
pub fn createCommand(self: *Yazap, cmd_name: []const u8) Command {
    return (Command.new(self.allocator, cmd_name));
}
